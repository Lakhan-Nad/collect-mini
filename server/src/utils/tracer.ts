...
export function recordProducerStart(
  id: TraceId,
  name: string,
  time: number
): void {
  tracer?.scoped(() => {
    tracer!.setId(id);
    tracer!.recordServiceName(LOCAL_SERVICE_NAME);
    tracer!.recordAnnotation(new Annotation.ProducerStart(), time);
    tracer!.recordAnnotation(new Annotation.LocalOperationStart(name), time);
  });
}

export function recordProducerStop(id: TraceId, time: number, err?: any): void {
  tracer?.scoped(() => {
    tracer!.setId(id);
    if (err !== undefined) {
      tracer!.recordBinary("error", JSON.stringify(err));
    }
    tracer!.recordAnnotation(new Annotation.ProducerStop(), time);
  });
}
